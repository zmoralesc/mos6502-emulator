...
const FLAG_NO_INTERRUPTS: u8 = 1 << 2;
...
impl<T: Bus> SerializableCpu<T> {
    pub fn from(cpu: &MOS6502<T>) -> Self {
        SerializableCpu {
            accumulator: cpu.accumulator,
            x_register: cpu.x_register,
            y_register: cpu.y_register,
...
fn make_opcode_array<T: Bus>() -> OpcodeFunctionArray<T> {
    [
...
impl<T: Bus> MOS6502<T> {
    ...
    pub fn save_to_file(&self, file: &mut File) -> Result<(), Box<dyn Error>> {
        let rep = SerializableCpu::from(self);
        let serializable_representation = serialize(&rep)?;
        file.write_all(&serializable_representation)?;
        Ok(())
    }
    ...
    pub fn from_file(file: &mut File, bus: T) -> Result<Self, Box<dyn Error>> {
        ...
        let serializable_representation: SerializableCpu<T> = deserialize(&buffer)?;

        Ok(Self {
            accumulator: serializable_representation.accumulator,
            x_register: serializable_representation.x_register,
            y_register: serializable_representation.y_register,
            program_counter: serializable_representation.program_counter,
            stack_pointer: serializable_representation.stack_pointer,
            status_register: serializable_representation.status_register,
            cycles: serializable_representation.cycles,
            bus,
            opcode_array: make_opcode_array(),
