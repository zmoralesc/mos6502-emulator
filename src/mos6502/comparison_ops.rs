...
macro_rules! compare_register {
    ($cpu:expr, $register:expr, $address_mode:ident) => {
        let operand: u8 = match $cpu.resolve_operand($address_mode)? {
            OpcodeOperand::Byte(b) => b,
            OpcodeOperand::Address(w) => $cpu.read_from_bus(w)?,
...
impl<T: Bus> MOS6502<T> {
    pub(super) fn cmp(&mut self, address_mode: AddressingMode) -> Result<(), EmulationError> {
        compare_register!(self, self.accumulator, address_mode);
    }

    pub(super) fn cpx(&mut self, address_mode: AddressingMode) -> Result<(), EmulationError> {
        compare_register!(self, self.x_register, address_mode);
    }

    pub(super) fn cpy(&mut self, address_mode: AddressingMode) -> Result<(), EmulationError> {
        compare_register!(self, self.y_register, address_mode);
    }
}
